...
if __name__ == "__main__":
    ...
    # 读取训练集和测试集数据
    with open('test_data_crest_azd.pkl', 'rb') as f:
    ...
    # 训练集和测试集的键
    train_keys = used_key[9:]
    test_keys = used_key[:9]
    ...
    #     '-0.5361657196+22.0980488*((HOMO_LUMO_Gap/((Quadrupole_Moment+Density)+sin(LEAmax))))+0.02378063701*((Nonpolar_Area/abs((Nonpolar_Area*HOMO_LUMO_Gap)-(Nonpolar_Area-ESPmax))))',
    ...
    if fine_tune:
        ...
        criterion = FormulaGuideRegressionLoss()
        optimizer = optim.Adam(list(model.parameters()) + [criterion.lambda_param], lr=0.0001)
        fine_tune_model(model, train_dataloader, test_dataloader, criterion, epochs=10000, patience=1000)
