...
def fine_tune_model(model, train_dataloader, val_dataloader, criterion, epochs=100, patience=100, use_early_stopping=False):
    """
    微调模型，解冻 FormulaLayer 的参数并仅优化该部分的权重。

    参数：
    - model: 训练好的模型
    - train_dataloader: 用于微调的训练数据加载器
    - test_dataloader: 用于验证的测试数据加载器
    - criterion: 损失函数
    - epochs: 微调的训练 epoch 数
    - lr: 微调时使用的学习率
    - model_path: 预训练模型的路径
    """
    # 定义 EarlyStopping 和损失函数
    best_r2 = -float('inf')  # 用来保存当前最佳的 R² 值
    early_stopping = None
    if use_early_stopping:
        # 初始化早停法
        early_stopping = EarlyStopping(patience=patience, verbose=True, metric_type='R2')
    ...
    for epoch in range(epochs):
        ...
        # 训练循环
        for batch in tqdm(train_dataloader, desc=f"Fine-tuning Epoch {epoch + 1}/{epochs}"):
            embeddings_batch, attributes_batch, targets_batch = batch
            embeddings_batch = {key: val.to(device) for key, val in embeddings_batch.items()}
            attributes_batch = attributes_batch.to(device)
            targets_batch = targets_batch.to(device)
    ...
    if early_stopping:
        early_stopping.restore_best_weights(model)
    print("Fine-tuning complete. Best model restored.")
...
if __name__ == "__main__":
    ...
    # 读取训练集和测试集数据
    with open('test_data_crest_BTK.pkl', 'rb') as f:
        train_data1 = pickle.load(f)
    # 将字典的键和对应的值按 5:5 划分为训练集和测试集
    keys = list(train_data1.keys())
    # random.shuffle(keys)  # 随机打乱键
    ...
    # 构建训练集和测试集字典
    train_data= {key: train_data1[key] for key in train_keys}
    test_data = {key: train_data1[key] for key in test_keys}
    ...
    formula_list = [
        '(((PSA/MPI)+(LEAmax/Pi))/ALIEmax)',
        '((LEAmax)**2/(PSA+Nonpolar_Area))',
        '(LEAmax/((PSA/LEAmin)+(ALIEmax*LEA_Ave)))'
    ]
    formula_signs = ['-', '+', '+']  # 注意：此处需要为每个公式添加符号


    # # 公式
    # formula = '-0.5361657196+22.0980488*((HOMO_LUMO_Gap/((Quadrupole_Moment+Density)+sin(LEAmax))))+0.02378063701*((Nonpolar_Area/abs((Nonpolar_Area*HOMO_LUMO_Gap)-(Nonpolar_Area-ESPmax))))'
    #
    # # 使用列表生成式将公式重复三次
    # formula_list = [formula] * 3  # 生成包含三次相同公式的列表
    #
    # # 每个公式的相关性符号 (+ 或 -)
    # formula_signs = ['+', '+', '+']  # 注意：此处需要为每个公式添加符号
    ...
    if fine_tune:
        ...
        # # 只解冻 FormulaLayer 中的参数
        # for param in model.formula_layer.parameters():
        #     param.requires_grad = True
        #
        # # alpha 是一个可微调的参数，确保它会被优化
        # model.alpha.requires_grad = True
        criterion = FormulaGuideRegressionLoss()
        optimizer = optim.Adam(list(model.parameters()) + [criterion.lambda_param], lr=0.00001)
        fine_tune_model(model, train_dataloader, test_dataloader, criterion, epochs=10000, patience=300,use_early_stopping=True)


    # 开始测试
    model.load_state_dict(torch.load('fine_tuned_best_model.pth'))
    true_values, predicted_values = test_model(model, test_dataloader)
    pearson_corr, _ = pearsonr(true_values, predicted_values)
    print(pearson_corr)
